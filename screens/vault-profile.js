...
export function render() {
      ...
      <div class="list-row">
        ...
        <div style="display:flex;align-items:center;gap:8px;">
          ...
          <div style="display:flex;gap:3px;">
            <button class="pill${(cp.sizeUnit||'cm')==='cm'?' sel':''}" id="vp-u-cm">cm</button>
            <button class="pill${cp.sizeUnit==='in'?' sel':''}" id="vp-u-in">in</button>
          </div>
        </div>
      </div>
    </div>
  ...
  (cp.additionalFeatures||[]).forEach(v => addlWrap.insertBefore(makeKwTag(v), addlWrap.querySelector('.add-tag')));
  const sensWrap = document.getElementById('vp-sensitive-wrap');
  (cp.sensitiveZones||[]).forEach(v => sensWrap.insertBefore(makeKwTag(v), sensWrap.querySelector('.add-tag')));
...
window.vpSwitchUnit = function(newUnit) {
  ...
  if (newUnit==='in' && _vpUnit==='cm' && v>0) sizeEl.value = Math.round(v/2.54*10)/10;
  else if (newUnit==='cm' && _vpUnit==='in' && v>0) sizeEl.value = Math.round(v*2.54*10)/10;
  _vpUnit = newUnit;
