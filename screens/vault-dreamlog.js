// screens/vault-dreamlog.js — Dream Log v2.6
...
function pickDreamType(chatText) {
  ...
  const weighted = DREAM_TYPES.map(t => ({
    ...t,
    w: t.weight + (boosts[t.type] || 0)
  }));
...
window.dlGenerate = async function() {
  ...
  const lv = setInterval(() => {
    ltEl.textContent = `${loadingMsgs[li++ % loadingMsgs.length]} 꿈 기록 중...`;
  }, 500);
  ...
  try {
    ...
    const typeInstructions = {
      nsfw:     `Write an NSFW dream. The dream must include explicit sexual content and desires. Explicit language and descriptions are fully allowed.`,
      comic:    `Write a comic dream. Something realistic but funny — a situation that could happen but is awkward or absurd. ${charName} is being serious inside the dream even though it's ridiculous.`,
      weird:    `Write a completely random nonsense dream with no logic. Something surreal and inexplicable — like ${userName} turning into an animal, or a dream that just ends for no reason.`,
      farewell: `Write an emotional, bittersweet dream about separation, longing, or not being able to reach ${userName}. Melancholic and heartfelt.`,
      fight:    `Write a dream where ${charName} and ${userName} fight or there's conflict. ${charName} loses or caves first in the end.`,
    };
...
Write STRICTLY in ${charName}'s exact voice, speech pattern, and personality as described in the character card.

Return ONLY a JSON object (no markdown):
{
  "title": "dream title — a punchy one-liner that captures the dream (Korean, max 10 chars)",
  "body": "dream content in ${charName}'s diary voice. exactly 10 lines. first-person, casual Korean. Explicit if NSFW.",
  "after": "2-4 lines. ${charName}'s reaction after waking up. If ${userName} is nearby, describe how ${charName} acts toward them. ${charReaction}"
}`;
