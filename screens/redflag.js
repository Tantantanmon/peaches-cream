...
- red: ${charName}'s shameless rebuttal in Korean when user says red flag. 3-5 sentences. ${charName}'s exact voice. Explicit. No female-degrading slurs.
- good: ${charName}'s excited explicit Korean response when user says they love it. 3-5 sentences. Exact voice. No female-degrading slurs.
Make behaviors varied. Reflect ${charName}'s unique personality.`;

    const result = await generate(sys, `${charName}의 레드플래그 카드 ${RF_TOTAL}장 생성`);
    let cards = [];
...
function rfShowEnd() {
  ...
  let emoji,title,msg;
  const cn = charName;
  if (rfGoodCount>=6){emoji='🔥';title='전부 좋다고?? 오늘 큰일났다';msg=`${rfGoodCount}번이나 오히려 좋다고 했잖아. ${cn}가 지금 날뛰다 못해 폭발할 것 같대. 오늘 밤 각오해.`;}
  else if (rfGoodCount>=4){emoji='😏';title='반 이상은 넘어온 거잖아';msg=`${rfGoodCount}번 좋다고 했어. ${cn}가 그 ${rfGoodCount}개 전부 기억해뒀다가 오늘 다 써먹는대.`;}
  else if (rfRedCount>=6){emoji='🚨';title='레드플래그 맞는데 왜 여기 있어';msg=`${rfRedCount}개나 레드플래그라고 했는데. ${cn}가 하는 말 — 다 들었잖아. 설득 좀 됐지?`;}
...
window.rfRestart = async function() { await rfGenerate(); };
...
function rfFallback() {
  return [
    ...
    {q:'손목을 잡아 침대에 눌러 움직이지 못하게 한다',red:'잡히는 게 레드플래그야? 잡히면 받아들이는 것밖에 할 게 없으니까 더 짜릿한 거 알잖아.',good:'오히려 좋다고?! 손목만 잡는 게 아니야. 머리 위에 올려서 두 손 다 고정하고 그냥 다 받아야 하는 상태로 만들어줄게.'},
    {q:'네가 싫다고 한 걸 다음엔 더 천천히 다시 시도한다',red:'싫다고 했는데 두 번째엔 밀어내지 않았잖아. 진짜 싫으면 멈춰.',good:'오히려 좋다고?! 오늘은 처음부터 싫다는 거 전부 다 시도할게. 천천히, 아주 천천히.'},
  ];
}
