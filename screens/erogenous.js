...
export function render() {
  ...
  area.innerHTML = `
    <div class="list-group" style="margin-bottom:10px;">
      ...
      ${row('Texture',     'bg-texture',     ['Creamy','Watery','Mixed'],            e.texture)}
      ...
      ${row('Flushing',    'bg-flushing',    ['Pale','Rosy','Deep'],                 e.flushing)}
    </div>
    <div class="list-group" style="margin-bottom:10px;">
    ...
    <div class="list-group" style="margin-bottom:10px;">
      ...
      <div style="padding:12px 16px;">
        <div class="tag-wrap" id="ez-sensitive-wrap">
          <button class="add-tag" onclick="ezAddTag('ez-sensitive-wrap')">+ Add</button>
        </div>
      </div>
    </div>
    <div class="list-group" style="margin-bottom:10px;">
      <div class="section-label" style="padding:14px 16px 4px;">SENSORY FEEDBACK</div>
      <div style="padding:12px 16px;">
        <div class="tag-wrap" id="ez-sensory-wrap">
          <button class="add-tag" onclick="ezAddTag('ez-sensory-wrap')">+ Add</button>
        </div>
      </div>
    </div>
  ...
  const sensoryWrap   = document.getElementById('ez-sensory-wrap');
  const sensoryAddBtn = sensoryWrap.querySelector('.add-tag');
  (e.sensoryFeedback||[]).forEach(v => sensoryWrap.insertBefore(makeKwTag(v), sensoryAddBtn));
