...
let todayIdx    = 0;
let pinnedCards = [];
let viewingPinIdx = -1; // 핀된 카드 열람 중 인덱스 (-1 = 오늘 카드 보는 중)
...
.ml-pinned-item{background:#fff;border:0.5px solid #e5e5ea;border-radius:16px;padding:16px 20px;display:flex;align-items:center;gap:14px;cursor:pointer;box-shadow:0 2px 8px rgba(0,0,0,0.04);transition:background .15s;}
.ml-pinned-item:active{background:#f9f9f9;}
...
The theme is: "${q.qEn}" (${q.qKr})

CRITICAL: Write as if this situation is ACTUALLY HAPPENING RIGHT NOW.
- Do NOT write "if this were to happen" or "I wonder what I'd say" type responses.
- Write as if ${charName} is IN that moment, speaking those words for real.
- Example: theme "the last time I see you" → write as if this IS the last time, and these are the actual words being said.
- The words must feel real, present, and emotionally grounded — not hypothetical.

Write STRICTLY in ${charName}'s exact voice and personality from the character card.

Return ONLY a JSON object (no markdown):
{
  "en": "3 lines of dialogue in English. First-person, spoken directly to ${userName}. Real, present-tense emotional weight. Each line on a new line.",
  "kr": "Korean translation of the English dialogue. 3 lines, same structure.",
  "mono": "2 lines of ${charName}'s inner thoughts in Korean after saying those words. ${charReaction}"
}`;
...
function renderAll() {
  ...
  // 핀된 카드 열람 중이면 그 카드 보여주기
  if (viewingPinIdx >= 0 && pinnedCards[viewingPinIdx]) {
    const p = pinnedCards[viewingPinIdx];
    wrap.innerHTML = `
      <div class="ml-card" id="ml-main-card">
        <div style="font-family:'Libre Baskerville',serif;font-size:10px;letter-spacing:2px;color:#aaa;text-transform:uppercase;margin-bottom:14px;">📌 Pinned</div>
        <div class="ml-q-en">${esc(p.qEn)}</div>
        <div class="ml-q-kr">${esc(p.qKr)}</div>
        <div class="ml-divider"></div>
        <div class="ml-en">${esc(p.en).replace(/\n/g,'<br>')}</div>
        <div class="ml-kr">${esc(p.kr).replace(/\n/g,'<br>')}</div>
        <div class="ml-divider2"></div>
        <div class="ml-mono">${esc(p.mono).replace(/\n/g,'<br>')}</div>
  ...
  if (todayCards.length > 1) {
    ...
    const otherLabel = todayCards[otherIdx].generated ? 'Next ✦' : 'Next ✦';
    html += `<button class="ml-nav-btn" onclick="mlNext()">${otherLabel}</button>`;
  }

  if (pinnedCards.length) {
    html += `<div class="ml-section-label">Archive · ${pinnedCards.length} / 10</div>`;
    html += `<div style="display:flex;flex-direction:column;gap:8px;">`;
    pinnedCards.forEach((p, i) => {
      html += `
...
window.mlOpenPin = function(i) {
  viewingPinIdx = i;
  renderAll();
};

window.mlClosePinView = function() {
  viewingPinIdx = -1;
  renderAll();
};
