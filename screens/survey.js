...
- options: array of exactly 4 answer choices in Korean. The 4th option must be "직접 입력..."
...
- comments: array of exactly 4 Korean responses (one per option index). Each 3-5 sentences. ${charName}'s exact voice reacting to that specific choice. Explicit. No female-degrading slurs. The 4th comment (index 3) should react to a custom/unexpected answer generically.`;
...
window.svChoose = function(idx) {
  const q = surveyQuestions[surveyCur];
  const isCustom = idx === q.options.length - 1 && q.options[idx] === '직접 입력...';

  if (isCustom) {
    const custom = prompt('직접 입력:');
    if (!custom || !custom.trim()) return;
    // 커스텀 입력 처리 — 버튼 선택 표시 후 idx 3 코멘트 사용
    document.querySelectorAll('.opt-btn').forEach((b,i)=>{ b.classList.toggle('selected',i===idx); b.style.pointerEvents='none'; });
    svShowReply(q, idx, custom.trim());
  } else {
    document.querySelectorAll('.opt-btn').forEach((b,i)=>{ b.classList.toggle('selected',i===idx); b.style.pointerEvents='none'; });
    svShowReply(q, idx, null);
  }
};

function svShowReply(q, idx, customText) {
  ...
  setTimeout(()=>{
    ...
    const comment = q.comments[idx] || q.comments[0];
    document.getElementById('sv-char-comment').textContent = customText
      ? `"${customText}"라고? ` + comment
      : comment;
...
function svFallback() {
  return [
    { q:'가장 좋아하는 체위는?', options:['Missionary','Doggy Style','Cowgirl','직접 입력...'],
      charAnswer:`Doggy. 뒤에서 네 허리 잡을 수 있잖아. 고개 돌려서 나 보려고 할 때 그냥 눌러버리는 게 좋거든. 다른 거 할 생각 없어.`,
      comments:['정상위? 눈 마주치는 거 좋아하는 거잖아. 숨기려고 해도 다 보이거든. 솔직히 나도 싫지 않아.','오, 나랑 같네. 기대해. 오늘 실망 안 시켜줄게.','카우걸이라고. 주도권 잡고 싶은 거야? 어디까지 할 수 있는지 보자.','그거라고? 말해줘서 고마워. 써먹을게.'] },
    { q:'관계 중 가장 흥분되는 순간은?', options:['상대가 신음할 때','눈이 마주칠 때','귀에 속삭일 때','직접 입력...'],
      charAnswer:`네가 참으려다 결국 소리 낼 때. 일부러 천천히 하는 이유가 그거야. 그 순간을 기다리는 거야.`,
      comments:['신음 소리... 그거 내가 만들어줄 수 있어. 오늘 확인해봐.','눈 마주치는 거 좋아해? 그럼 오늘은 내내 눈 못 피하게 할게.','귀에 속삭이는 거라면 — 할 말이 많은데. 준비해.','그게 흥분된다고? 기억해뒀어.'] },
    { q:'끝나고 원하는 건?', options:['그냥 안겨있기','바로 잠들기','한 번 더','직접 입력...'],
      charAnswer:`솔직히 — 그냥 네 옆에 있고 싶어. 말 안 해도 되니까. 끝나고 네 숨소리 들으면서 있는 게 이상하게 좋더라.`,
      comments:['안겨있고 싶어? ...그래. 그거라면.','바로 잠든다고? 나 옆에 있는데? 섭섭하네.','한 번 더라고. 체력은 있네. 오늘 확인해볼게.','그게 하고 싶어? 끝나고도 내 곁에 있고 싶은 거잖아.'] },
    { q:'가장 약한 부위는?', options:['목','귀','허리','직접 입력...'],
      charAnswer:`목. 거기 건드리면 나도 이성 잃어. 알면서 건드리면 어떻게 되는지는 직접 확인해봐.`,
      comments:['목이라고. 기억해뒀어.','귀라고. 그럼 오늘 귀에 집중할게.','허리. 잡으면 되겠네.','솔직하게 말해줘서 고마워. 써먹을게.'] },
    { q:'분위기는 어떤 게 더 좋아?', options:['천천히, 감정적으로','빠르고 거칠게','뜸들이면서 길게','직접 입력...'],
      charAnswer:`빠르고 거칠게. 근데 가끔 네가 울 것 같은 표정 지을 때는 천천히 해주고 싶어.`,
      comments:['천천히 감정적으로. 그런 날도 있어. 맞춰줄게.','거칠게라고. 오늘 마음에 드는 답 들었어.','뜸들이는 거 좋아해? 나도 그거 좋아.','그럼 내가 읽어줄게. 매번.'] },
  ];
}
