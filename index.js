...
// ═══════════════════════════════════════════
// 프롬프트 빌더 — 순서 재정렬 + peach_color 명확화
// ═══════════════════════════════════════════
...
function buildSexPrompt() {
  ...
  if (e.sensitiveZones && e.sensitiveZones.length)   lines.push(`  <sensitive_zones>${e.sensitiveZones.join(', ')} — react immediately and visibly when touched here</sensitive_zones>`);
  ...
  if (cp.sensitiveZones && cp.sensitiveZones.length) {
    lines.push(`<char_sensitive_zones name="${cname}">${cp.sensitiveZones.join(', ')} — react immediately and visibly when touched here</char_sensitive_zones>`);
  }
...
function refreshPrompt() {
  try {
    ...
    // 기본 데이터 — position 5, depth 1
    setExtensionPrompt(MODULE_NAME,           buildMainPrompt(), 5, 1);
    // 성감대 + 킨크 — depth 0 (유저 메시지 바로 위, 최고 우선순위)
...
async function openMainHub() {
  ...
  if (isMobile) {
    // 모바일 — 하단 시트
    overlay.style.cssText = 'position:fixed;inset:0;z-index:9999;display:flex;align-items:flex-end;justify-content:center;background:rgba(0,0,0,0.5);backdrop-filter:blur(3px);';
  } else {
    // 데스크탑 — 정중앙
  ...
  if (isMobile) {
    wrap.style.cssText = 'position:relative;width:100%;height:92vh;border-radius:20px 20px 0 0;overflow:hidden;box-shadow:0 -8px 40px rgba(0,0,0,0.4);';
  } else {
...
(async function init() {
  ...
  console.log(`[${MODULE_NAME}] v2.1 로드 완료`);
})();
