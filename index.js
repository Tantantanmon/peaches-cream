...
function refreshPrompt() {
  try {
    ...
    // position 0 = 시스템 프롬프트 맨 앞 (최고 우선순위)
    setExtensionPrompt(MODULE_NAME,        buildMainPrompt(), 0, 0);
...
window.pcTbApply=async function(){
  ...
  if(byKey.foreplayActs?.length)   parts.push(`Foreplay: ${byKey.foreplayActs.join(', ')}`);
  if(byKey.pace?.length)           parts.push(`Pace: ${byKey.pace.join(', ')}`);
  if(byKey.toys?.length)           parts.push(`Toys: ${byKey.toys.join(', ')}`);
  ...
  if(byKey.orgasm?.length)         parts.push(`Orgasm: ${byKey.orgasm.join(', ')}`);
  if(condom==='on')                parts.push('Use condom');
  ...
  try{
    ...
    setTimeout(()=>{ try{setExtensionPrompt(MODULE_NAME+'_action','',1,0);}catch(e){} },500);
...
(async function init(){
  ...
  console.log(`[${MODULE_NAME}] v2.5 로드 완료`);
})();
