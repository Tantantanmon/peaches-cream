...
const defaultGlobalConfig = {
  ...
  customTags: {
    mood: [], location: [], foreplayActs: [], pace: [],
    toys: [], positions: [], action: [], finish: [], sensitiveZones: [], orgasm: []
  }
};
...
const APP_TOKENS = {
  redflag: 600, clinic: 800, reviews: 900,
  offrecord: 1000, fanfeed: 1200, dreamlog: 600, monologue: 600,
};
...
function getStore() {
  ...
  if (!existing || existing.version !== DATA_VERSION) {
    extensionSettings[MODULE_NAME] = {
      ...
      config: JSON.parse(JSON.stringify(defaultGlobalConfig)),
      chars: {}
    };
  }
  ...
  Object.keys(defaultGlobalConfig).forEach(k => {
    if (s.config[k] === undefined) s.config[k] = JSON.parse(JSON.stringify(defaultGlobalConfig[k]));
  });
...
function getCharStore() {
  ...
  Object.keys(defaultCharData).forEach(k => {
    if (cd[k] === undefined) cd[k] = JSON.parse(JSON.stringify(defaultCharData[k]));
  });
  return cd;
}
...
function getUserPersona() {
  try { const c = ctx(); return c.persona || c?.powerUserSettings?.persona_description || ''; }
  catch(e) { return ''; }
}

// ── GENERATE WRAPPER ───────────────────────────────────────────────────────
// generateRaw만 사용 — ST 내부 파이프라인(임베딩/벡터 훅) 완전 우회
  ...
  if (apiSrc.startsWith('profile:')) {
    try {
      ...
      if (cmrs && typeof cmrs.loadProfile === 'function') {
        await cmrs.loadProfile(apiSrc.replace('profile:', ''));
      }
    } catch(e) {}
  }
...
function getRecentChat(limit) {
  try {
    const { chat } = ctx();
    return (chat || []).slice(-(limit || 10)).map(m => ({
...
function getChatRange(s, e) {
  try {
    ...
    const si = s ? Math.max(1, parseInt(s)) : 1;
    const ei = e ? Math.min(arr.length, parseInt(e)) : arr.length;
    return arr.slice(si - 1, ei).map(m => ({
      role: m.is_user ? 'user' : 'assistant',
      content: m.mes || '',
      name: m.name || ''
    }));
...
function buildToolbarHTML() {
  ...
  const groupsHTML = GROUPS.map(g => {
    ...
    const subsHTML = g.subs.map(sub => {
      ...
      const items  = sub.key === 'sensitiveZones' ? [] : [...fixed, ...custom];
...
window.pcTbApply = async function() {
  ...
  try {
    const c = ctx();
    // setExtensionPrompt으로 인젝션 후 generateRaw 사용 (벡터/임베딩 파이프라인 우회)
    c.setExtensionPrompt(MODULE_NAME + '_action', actionMsg, 1, 0);
    const charDesc = getCharDescription();
    const sysPrompt = charDesc
      ? `${charDesc}\n\n${actionMsg}`
      : actionMsg;
    await c.generateRaw({ systemPrompt: sysPrompt, prompt: '', max_new_tokens: store.config.maxTokens || 1500 });
    setTimeout(() => {
      try { c.setExtensionPrompt(MODULE_NAME + '_action', '', 1, 0); } catch(e) {}
    }, 300);
  } catch(e) {
    console.error(`[${MODULE_NAME}] apply error`, e);
  }
};
...
(async function init() {
  ...
  console.log(`[${MODULE_NAME}] v2.7 로드 완료`);
})();
