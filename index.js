...
async function generateWithRole(systemPrompt, userPrompt, appName) {
  ...
  const tokens = (appName && APP_TOKENS[appName])
    ? APP_TOKENS[appName]
    : (store.config.maxTokens || 1500);
  try {
    // 메시지 배열 구성
    const messages = [
    ...
    // SillyTavern의 generate 사용
    const response = await c.generateQuietPrompt(userPrompt, false, false, '', systemPrompt);

    return response || '';

  } catch(e) {
    console.error('[PC] Generate error:', e);

    // Fallback: 기본 generate 사용
    try {
      return await c.generate(userPrompt, { amount: tokens }) || '';
    } catch(e2) {
      console.error('[PC] Fallback generate error:', e2);
      throw new Error('API 호출에 실패했습니다. Main API 연결을 확인해주세요.');
    }
  }
}
