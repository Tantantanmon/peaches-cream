// screens/vault-offrecord.js — Off the Record
...
export async function render() {
  ...
  `;
  document.getElementById('popup').insertBefore(tabBar, document.querySelector('.hdivider').nextSibling);

  area.innerHTML = `
    <!-- COG -->
    <div id="otr-pane-cog" style="display:flex;flex-direction:column;gap:10px;">
      <div class="otr-card" id="cog-card">
        <div class="otr-num" id="cog-num">1 / 6</div>
    ...
    <div id="otr-pane-dark" style="display:none;flex-direction:column;gap:10px;">
      <div class="otr-card" id="dark-card">
        <div class="otr-num" id="dark-num">1 / 6</div>
  ...
  // 저장된 카드 로드
  syncStore();
  if (store.cogCards  && store.cogCards.length)  { cogCards  = store.cogCards;  renderCard('cog');  }
  if (store.darkCards && store.darkCards.length) { darkCards = store.darkCards; renderCard('dark'); }
}
...
window.otrPrev = function(type) {
  ...
  else              darkIdx = (darkIdx-1+cards.length) % cards.length;
  renderCard(type);
};
...
- comment: 2-4 line Korean comment in ${charName}'s voice (italic-style inner monologue, slightly defensive, mix of denial and admission). NO female-degrading slurs.`;
    } else {
...
- comment: 2-4 line Korean comment in ${charName}'s exact voice. Explicit, unapologetic, shows character personality. NO female-degrading slurs.`;
    }

    const result = await generate(sys, `카드 6장 생성해줘`);
    let cards = [];
    try { cards = JSON.parse(result.replace(/```json|```/g,'').trim()); } catch(e) {}
    if (!Array.isArray(cards)||!cards.length) { alert('생성에 실패했어요.'); loading.style.display='none'; return; }

    const newHistory = [...history, ...cards.map(c=>c.text)].slice(-36);
