...
export function render() {
    ...
    <div class="list-group" style="margin-bottom:10px;">
      ...
      <div class="list-row">
        ...
        <div class="num-row">
          ...
          <div class="btn-group" id="vp-bg-unit">
            <button class="pill${(cp.sizeUnit||'cm')==='cm'?' sel':''}" data-val="cm">cm</button>
            <button class="pill${cp.sizeUnit==='in'?' sel':''}" data-val="in">in</button>
          </div>
        </div>
      </div>
    </div>
  ...
  // unit 스위치
  document.getElementById('vp-bg-unit').addEventListener('click', e => {
    const b = e.target.closest('.pill');
    if (!b) return;
    const newUnit = b.dataset.val;
    const oldUnit = document.querySelector('#vp-bg-unit .pill.sel')?.dataset.val || 'cm';
    const sizeEl  = document.getElementById('vp-size');
    const v = parseFloat(sizeEl.value) || 0;
    if (newUnit === 'in' && oldUnit === 'cm') sizeEl.value = Math.round(v / 2.54 * 10) / 10;
    else if (newUnit === 'cm' && oldUnit === 'in') sizeEl.value = Math.round(v * 2.54 * 10) / 10;
    document.getElementById('vp-bg-unit').querySelectorAll('.pill').forEach(x => x.classList.remove('sel'));
    b.classList.add('sel');
  });
  ...
  document.getElementById('vp-save-btn').onclick = save;
}
...
function save() {
  const sizeUnit = document.querySelector('#vp-bg-unit .pill.sel')?.dataset.val || 'cm';
  const data = {
    size:               document.getElementById('vp-size')?.value || '',
    sizeUnit,
    additionalFeatures: getKwTags('vp-addl-wrap'),
    moaning:            getSelected('vez-bg-moaning'),
    semen:              getSelected('vez-bg-semen'),
    multiRound:         getSelected('vez-bg-multiRound'),
    stamina:            getSelected('vez-bg-stamina'),
